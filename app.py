...
# ──────────────────────────────────────────────
# JSON 스키마 (Gemini 응답 구조 강제)
# ──────────────────────────────────────────────

ANALYSIS_SCHEMA = {
    "type": "object",
    "properties": {
        "total_duration": {"type": "string", "description": "MM:SS 형식"},
        "scenes": {
            "type": "array",
            "items": {
                "type": "object",
                "properties": {
                    ...
                    "shot_type": {
                        ...
                        "enum": [
                            ...
                            "POV Shot", "Two Shot", "Insert Shot", "Cutaway",
                        ],
                    },
                    "camera_movement": {
                        "type": "string",
                        "enum": [
                            ...
                            "Dolly In", "Dolly Out", "Tracking/Follow",
                    ...
                    "subject_type": {
                        "type": "string",
                        "enum": [
                            "speaker", "audience", "product_demo",
                            "venue_exterior", "venue_interior",
                            "signage_branding", "networking", "B-roll",
                            "title_card", "transition_graphic",
                        ],
                    },
                    "editorial_purpose": {
                        "type": "string",
                        "enum": [
                            "establish_context", "introduce_speaker",
                            "build_energy", "deliver_information",
                            "show_reaction", "emotional_beat",
                            "transition", "pacing_break", "closing",
                        ],
                    },
                    "description": {"type": "string"},
                },
                "required": ["index", "start", "end", "shot_type", "description"],
            },
        },
        "editing_patterns": {
            "type": "object",
            "properties": {
                "avg_cut_duration_sec": {"type": "number"},
                "pacing_curve": {"type": "string"},
                "shot_type_distribution": {"type": "object"},
                "recurring_sequences": {
                    "type": "array",
                    "items": {"type": "string"},
                },
                "dominant_camera_style": {"type": "string"},
            },
        },
        "narrative_structure": {"type": "string"},
        "summary": {"type": "string"},
    },
    "required": ["total_duration", "scenes", "editing_patterns", "narrative_structure", "summary"],
}
...
def retry_with_backoff(max_retries=3, base_delay=10, max_delay=60):
    """지수 백오프 재시도 데코레이터. Rate limit/서버 에러 시 자동 재시도."""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            for attempt in range(max_retries + 1):
                try:
                    ...
                    retriable = any(kw in error_str for kw in [
                        "429", "503", "500",
                    ...
                    st.warning(f"API 에러 (시도 {attempt + 1}/{max_retries + 1}): {e}")
                    st.info(f"{delay}초 후 재시도...")
                    time.sleep(delay)
...
def run_batch_analysis(
    ...
    for i, video_info in enumerate(videos):
        ...
        # 배치 간 딜레이 (rate limit 방지, 무료 티어 10 RPM 기준)
        if i < total - 1:
            time.sleep(6)
